...
void CANInterpretRx() {
    ...
    if (rx_frame.Rx_header.IDE == CAN_ID_EXT) { // 29-bit CAN messages
        ...
        // ===== PM100DX Inverter CAN IDs =====
        if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0A0)) {
            // Inverter Temperatures #1 - Address = 0x0A0
            CAN_Parse_Inverter_Temp1(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0A1)) {
            // Inverter Temperatures #2 - Address = 0x0A1
            CAN_Parse_Inverter_Temp2(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0A2)){
            // Inverter Temperatures #3 and Torque Shudder - Address = 0x0A2
            CAN_Parse_Inverter_Temp3TorqueShudder(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0A4)){
            // Inverter Digital Input Status - Address = 0x0A4
            CAN_Parse_Inverter_DigitalInputStatus(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0A5)){
            // Inverter Motor Position Parameters - Address = 0x0A5
            CAN_Parse_Inverter_MotorPositionParameters(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0A6)){
            // Inverter Current Parameters - Address = 0x0A6
            CAN_Parse_Inverter_CurrentParameters(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0A7)){
            // Inverter Voltage Parameters - Address = 0x0A7
            CAN_Parse_Inverter_VoltageParameters(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0A8)){
            // Inverter Flux Parameters - Address = 0x0A8
            CAN_Parse_Inverter_FluxParameters(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0A9)){
            // Inverter Internal Voltage Parameters - Address = 0x0A9
            CAN_Parse_Inverter_InternalVoltageParameters(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0AA)){
            // Inverter Internal State Parameters - Address = 0x0AA
            CAN_Parse_Inverter_InternalStateParameters(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0AB)){
            // Inverter Fault Codes - Address = 0x0AB
            CAN_Parse_Inverter_FaultCodes(rx_frame);
        } else if (rx_frame.Rx_header.ExtId == ((CAN_INVERTER_BASE_ID << 16) | 0x0B0)){
            // Inverter High Speed Parameters - Address = 0x0B0
            CAN_Parse_Inverter_HighSpeedParameters(rx_frame);
        }
    }
}
...
void CAN_Parse_Inverter_Temp1(CAN_Queue_Frame_t frame) {
    ...
    CVC_data[INVERTER_POWER_MODULE_A_TEMP].data = (frame.data[0]<<8) | frame.data[1];
    CVC_data[INVERTER_POWER_MODULE_A_TEMP].type = INT;
    // Byte 2, 3: Power Module Phase B Temperature
    CVC_data[INVERTER_POWER_MODULE_B_TEMP].data = (frame.data[2]<<8) | frame.data[3];
    CVC_data[INVERTER_POWER_MODULE_B_TEMP].type = INT;
    // Byte 4, 5: Power Module Phase  C Temperature
    CVC_data[INVERTER_POWER_MODULE_C_TEMP].data = (frame.data[4]<<8) | frame.data[5];
    CVC_data[INVERTER_POWER_MODULE_C_TEMP].type = INT;
    // Byte 6, 7: Gate Driver Board Temperature
    CVC_data[INVERTER_GATE_DRIVER_BOARD_TEMP].data = (frame.data[6]<<8) | frame.data[7];
    CVC_data[INVERTER_GATE_DRIVER_BOARD_TEMP].type = INT;
    xSemaphoreGive(CVC_DataMutex);
}
...
void CAN_Parse_Inverter_Temp2(CAN_Queue_Frame_t frame) {
    ...
    CVC_data[INVERTER_CONTROL_BOARD_TEMP].data = (frame.data[0]<<8) | frame.data[1];
    CVC_data[INVERTER_CONTROL_BOARD_TEMP].type = INT;
    // Byte 2, 3: Temperature read from RTD input #1
    CVC_data[INVERTER_RTD_INPUT_1_TEMP].data = (frame.data[2]<<8) | frame.data[3];
    CVC_data[INVERTER_RTD_INPUT_1_TEMP].type = INT;
    // Byte 4, 5: Temperature read from RTD Input #2
    CVC_data[INVERTER_RTD_INPUT_2_TEMP].data = (frame.data[4]<<8) | frame.data[5];
    CVC_data[INVERTER_RTD_INPUT_2_TEMP].type = INT;
    // Byte 6, 7: Stall Burst Model Temperature 
    CVC_data[INVERTER_STALL_BURST_MODEL_TEMP].data = (frame.data[6]<<8) | frame.data[7];
    CVC_data[INVERTER_STALL_BURST_MODEL_TEMP].type = INT;
    xSemaphoreGive(CVC_DataMutex);
}
/**
...
void CAN_Parse_Inverter_Temp3TorqueShudder(CAN_Queue_Frame_t frame){
    ...
    CVC_data[INVERTER_COOLANT_TEMP].data = (frame.data[0]<<8) | frame.data[1];
    CVC_data[INVERTER_COOLANT_TEMP].type = INT;
    // Byte 2, 3: Estimated hot spot temperature internal to inverter.
    CVC_data[INVERTER_HOT_SPOT_TEMP].data = (frame.data[2]<<8) | frame.data[3];
    CVC_data[INVERTER_HOT_SPOT_TEMP].type = INT;
    // Byte 4, 5: Filtered temperature value from the motor temperature sensor
    CVC_data[INVERTER_MOTOR_TEMP].data = (frame.data[4]<<8) | frame.data[5];
    CVC_data[INVERTER_MOTOR_TEMP].type = INT;
    // Byte 6, 7: A value of torque used in shudder compensation.
    CVC_data[INVERTER_TORQUE_SHUDDER].data = (frame.data[6]<<8) | frame.data[7];
    CVC_data[INVERTER_TORQUE_SHUDDER].type = INT;
    xSemaphoreGive(CVC_DataMutex);
}
...
void CAN_Parse_Inverter_DigitalInputStatus(CAN_Queue_Frame_t frame){
    ...
    CVC_data[INVERTER_FORWARD_SWITCH].data = frame.data[0] ;
    CVC_data[INVERTER_FORWARD_SWITCH].type = BOOLEAN;
    // Byte 1: Status of Digital Input #2, Reverse switch
    CVC_data[INVERTER_REVERSE_SWITCH].data = frame.data[1] ;
    CVC_data[INVERTER_REVERSE_SWITCH].type = BOOLEAN;
    // Byte 2: Status of Digital Input #3, Reverse switch
    CVC_data[INVERTER_BRAKE_SWITCH].data = frame.data[2] ;
    CVC_data[INVERTER_BRAKE_SWITCH].type = BOOLEAN;
    // Byte 3: Status of Digital Input #4, Regen Disable switch
    CVC_data[INVERTER_REGEN_DISABLE_SWITCH].data = frame.data[3] ;
    CVC_data[INVERTER_REGEN_DISABLE_SWITCH].type = BOOLEAN;
    // Byte 4: Status of Digital Input #5, Ignition switch
    CVC_data[INVERTER_IGNITION_SWITCH].data = frame.data[4] ;
    CVC_data[INVERTER_IGNITION_SWITCH].type = BOOLEAN;
    // Byte 5: Status of Digital Input #6, Start switch
    CVC_data[INVERTER_START_SWITCH].data = frame.data[5] ;
    CVC_data[INVERTER_START_SWITCH].type = BOOLEAN;
    // Byte 6: Status of Digital Input #7, Valet Mode
    CVC_data[INVERTER_VALET_MODE].data = frame.data[6] ;
    CVC_data[INVERTER_VALET_MODE].type = BOOLEAN;
    xSemaphoreGive(CVC_DataMutex);
}
...
void CAN_Parse_Inverter_MotorPositionParameters(CAN_Queue_Frame_t frame){
    ...
    CVC_data[INVERTER_MOTOR_ANGLE].data = (frame.data[0]<<8) | frame.data[1];
    CVC_data[INVERTER_MOTOR_ANGLE].type = INT;
    // Byte 2, 3: Motor Speed
    CVC_data[INVERTER_MOTOR_SPEED].data = (frame.data[2]<<8) | frame.data[3];
    CVC_data[INVERTER_MOTOR_SPEED].type = INT;
    //Byte 4, 5: Electrical Output Frequency
    CVC_data[INVERTER_ELECTRICAL_OUTPUT_FREQUENCY].data = (frame.data[4]<<8) | frame.data[5];
    CVC_data[INVERTER_ELECTRICAL_OUTPUT_FREQUENCY].type = INT;
    //Byte 6, 7: Delta Resolver Filtered
    CVC_data[INVERTER_DELTA_RESOLVER_FILTERED].data = (frame.data[6]<<8) | frame.data[7];
    CVC_data[INVERTER_DELTA_RESOLVER_FILTERED].type = INT;
    xSemaphoreGive(CVC_DataMutex);
}
...
void CAN_Parse_Inverter_CurrentParameters(CAN_Queue_Frame_t frame){
    ...
    CVC_data[INVERTER_PHASE_A_CURRENT].data = (frame.data[0]<<8) | frame.data[1];
    CVC_data[INVERTER_PHASE_A_CURRENT].type = INT;
    // Byte 2, 3: Phase B Current
    CVC_data[INVERTER_PHASE_B_CURRENT].data = (frame.data[2]<<8) | frame.data[3];
    CVC_data[INVERTER_PHASE_B_CURRENT].type = INT;
    // Byte 4, 5: Phase C Current
    CVC_data[INVERTER_PHASE_C_CURRENT].data = (frame.data[4]<<8) | frame.data[5];
    CVC_data[INVERTER_PHASE_C_CURRENT].type = INT;
    // Byte 6, 7: DC BUS Current
    CVC_data[INVERTER_DC_BUS_CURRENT].data = (frame.data[6]<<8) | frame.data[7];
    CVC_data[INVERTER_DC_BUS_CURRENT].type = INT;
    xSemaphoreGive(CVC_DataMutex);
}
...
void CAN_Parse_Inverter_VoltageParameters(CAN_Queue_Frame_t frame){
    ...
    CVC_data[INVERTER_DC_BUS_VOLTAGE].data = (frame.data[0]<<8) | frame.data[1];
    CVC_data[INVERTER_DC_BUS_VOLTAGE].type = INT;
    // Byte 2, 3: The calculated value of the output voltage, in peak line-neutral volts
    CVC_data[INVERTER_OUTPUT_VOLTAGE].data = (frame.data[2]<<8) | frame.data[3];
    CVC_data[INVERTER_OUTPUT_VOLTAGE].type = INT;
    ...
    CVC_data[INVERTER_VAB_VD_VOLTAGE].data = (frame.data[4]<<8) | frame.data[5];
    CVC_data[INVERTER_VAB_VD_VOLTAGE].type = INT;
    // Byte 6, 7: DC BUS Current
    CVC_data[INVERTER_VAB_VQ_VOLTAGE].data = (frame.data[6]<<8) | frame.data[7];
    CVC_data[INVERTER_VAB_VQ_VOLTAGE].type = INT;
    xSemaphoreGive(CVC_DataMutex);
}
...
void CAN_Parse_Inverter_FluxParameters(CAN_Queue_Frame_t frame){
    ...
    CVC_data[INVERTER_FLUX_COMMAND].data = (frame.data[0]<<8) | frame.data[1];
    CVC_data[INVERTER_FLUX_COMMAND].type = INT;
    // Byte 2, 3: The estimated flux
    CVC_data[INVERTER_FLUX_FEEDBACK].data = (frame.data[2]<<8) | frame.data[3];
    CVC_data[INVERTER_FLUX_FEEDBACK].type = INT;
    // Byte 4, 5: D-axis current feedback
    CVC_data[INVERTER_ID_CURRENT].data = (frame.data[4]<<8) | frame.data[5];
    CVC_data[INVERTER_ID_CURRENT].type = INT;
    // Byte 6, 7: Q-axis current feedback
    CVC_data[INVERTER_IQ_CURRENT].data = (frame.data[6]<<8) | frame.data[7];
    CVC_data[INVERTER_IQ_CURRENT].type = INT;
    xSemaphoreGive(CVC_DataMutex);
}
...
void CAN_Parse_Inverter_InternalVoltageParameters(CAN_Queue_Frame_t frame){
    ...
    CVC_data[INVERTER_1_5_REFERENCE_VOLTAGE].data = (frame.data[0]<<8) | frame.data[1];
    CVC_data[INVERTER_1_5_REFERENCE_VOLTAGE].type = INT;
    // Byte 2, 3: 2.5 Reference Voltage
    CVC_data[INVERTER_2_5_REFERENCE_VOLTAGE].data = (frame.data[2]<<8) | frame.data[3];
    CVC_data[INVERTER_2_5_REFERENCE_VOLTAGE].type = INT;
    // Byte 4, 5: 5.0 Reference Voltage
    CVC_data[INVERTER_5_0_REFERENCE_VOLTAGE].data = (frame.data[4]<<8) | frame.data[5];
    CVC_data[INVERTER_5_0_REFERENCE_VOLTAGE].type = INT;
    // Byte 6, 7: 12.0 Reference Voltage
    CVC_data[INVERTER_12_0_REFERENCE_VOLTAGE].data = (frame.data[6]<<8) | frame.data[7];
    CVC_data[INVERTER_12_0_REFERENCE_VOLTAGE].type = INT;
    xSemaphoreGive(CVC_DataMutex);
}
...
void CAN_Parse_Inverter_InternalStateParameters(CAN_Queue_Frame_t frame){
    xSemaphoreTake(CVC_DataMutex, portMAX_DELAY);
    // Byte 0: VSM State
    CVC_data[INVERTER_VSM_STATE].data = frame.data[0];
    CVC_data[INVERTER_VSM_STATE].type = UINT;
    // Byte 1: PWM Frequency
    CVC_data[INVERTER_PWM_FREQUENCY].data = frame.data[1];
    CVC_data[INVERTER_PWM_FREQUENCY].type = INT;
    // Byte 2: Inverter State
    CVC_data[INVERTER_STATE].data = frame.data[2];
    CVC_data[INVERTER_STATE].type = UINT;
    // Byte 3: Relay state
    ...
    CVC_data[INVERTER_RELAY_1_STATUS].data = (frame.data[3] >> 0) & 0x01;
    CVC_data[INVERTER_RELAY_1_STATUS].type = BOOLEAN;
    CVC_data[INVERTER_RELAY_2_STATUS].data = (frame.data[3] >> 1) & 0x01;
    CVC_data[INVERTER_RELAY_2_STATUS].type = BOOLEAN;
    CVC_data[INVERTER_RELAY_3_STATUS].data = (frame.data[3] >> 2) & 0x01;
    CVC_data[INVERTER_RELAY_3_STATUS].type = BOOLEAN;
    CVC_data[INVERTER_RELAY_4_STATUS].data = (frame.data[3] >> 3) & 0x01;
    CVC_data[INVERTER_RELAY_4_STATUS].type = BOOLEAN;
    CVC_data[INVERTER_RELAY_5_STATUS].data = (frame.data[3] >> 4) & 0x01;
    CVC_data[INVERTER_RELAY_5_STATUS].type = BOOLEAN;
    CVC_data[INVERTER_RELAY_6_STATUS].data = (frame.data[3] >> 5) & 0x01;
    CVC_data[INVERTER_RELAY_6_STATUS].type = BOOLEAN;
    // Byte 4
    ...
    CVC_data[INVERTER_RUN_MODE].data = (frame.data[4] >> 0) & 0x01;
    CVC_data[INVERTER_RUN_MODE].type = BOOLEAN;
    CVC_data[INVERTER_SELF_SENSING_ASSIST].data = (frame.data[4] >> 1) & 0x01;
    CVC_data[INVERTER_SELF_SENSING_ASSIST].type = BOOLEAN;
    CVC_data[INVERTER_ACTIVE_DISCHARGE_STATE].data = (frame.data[4] >> 5) & 0x07;
    CVC_data[INVERTER_ACTIVE_DISCHARGE_STATE].type = UINT;
    // Byte 5
    // Bit 0: Inverter Command Mode
    // Bit 4-7: Rolling Counter Value
    CVC_data[INVERTER_COMMAND_MODE].data = (frame.data[5] >> 0) & 0x01;
    CVC_data[INVERTER_COMMAND_MODE].type = BOOLEAN;
    CVC_data[INVERTER_ROLLING_COUNTER_VALUE].data = (frame.data[5] >> 4) & 0x0F;
    CVC_data[INVERTER_ROLLING_COUNTER_VALUE].type = UINT;
    // Byte 6
    ...
    CVC_data[INVERTER_ENABLE_STATE].data = (frame.data[6] >> 0) & 0x01;
    CVC_data[INVERTER_ENABLE_STATE].type = BOOLEAN;
    CVC_data[INVERTER_BURST_MODEL_MODE].data = (frame.data[6] >> 0) & 0x01;
    CVC_data[INVERTER_BURST_MODEL_MODE].type = BOOLEAN;
    CVC_data[INVERTER_START_MODE_ACTIVE].data = (frame.data[6] >> 6) & 0x01;
    CVC_data[INVERTER_START_MODE_ACTIVE].type = BOOLEAN;
    CVC_data[INVERTER_ENABLE_LOCKOUT].data = (frame.data[6] >> 7) & 0x01;
    CVC_data[INVERTER_ENABLE_LOCKOUT].type = BOOLEAN;
    // Byte 7
    ...
    CVC_data[INVERTER_DIRECTION_COMMAND].data = (frame.data[7] >> 0) & 0x01;
    CVC_data[INVERTER_DIRECTION_COMMAND].type = BOOLEAN;
    CVC_data[INVERTER_BMS_ACTIVE].data = (frame.data[7] >> 1) & 0x01;
    CVC_data[INVERTER_BMS_ACTIVE].type = BOOLEAN;
    CVC_data[INVERTER_BMS_LIMITING_TORQUE].data = (frame.data[7] >> 2) & 0x01;
    CVC_data[INVERTER_BMS_LIMITING_TORQUE].type = BOOLEAN;
    CVC_data[INVERTER_LIMIT_MAX_SPEED].data = (frame.data[7] >> 3) & 0x01;
    CVC_data[INVERTER_LIMIT_MAX_SPEED].type = BOOLEAN;
    CVC_data[INVERTER_LIMIT_HOT_SPOT].data = (frame.data[7] >> 4) & 0x01;
    CVC_data[INVERTER_LIMIT_HOT_SPOT].type = BOOLEAN;
    CVC_data[INVERTER_LOW_SPEED_LIMITING].data = (frame.data[7] >> 5) & 0x01;
    CVC_data[INVERTER_LOW_SPEED_LIMITING].type = BOOLEAN;
    CVC_data[INVERTER_COOLANT_TEMP_LIMITING].data = (frame.data[7] >> 6) & 0x01;
    CVC_data[INVERTER_COOLANT_TEMP_LIMITING].type = BOOLEAN;
    CVC_data[INVERTER_LIMIT_STALL_BURST_MODEL].data = (frame.data[7] >> 7) & 0x01;
    CVC_data[INVERTER_LIMIT_STALL_BURST_MODEL].type = BOOLEAN;
    xSemaphoreGive(CVC_DataMutex);
}
...
void CAN_Parse_Inverter_FaultCodes(CAN_Queue_Frame_t frame){
    ...
    CVC_data[INVERTER_POST_FAULT_LO].data = (frame.data[0]<<8) | frame.data[1];
    CVC_data[INVERTER_POST_FAULT_LO].type = UINT;
    // Byte 2, 3: POST Fault Hi
    CVC_data[INVERTER_POST_FAULT_HI].data = (frame.data[2]<<8) | frame.data[3];
    CVC_data[INVERTER_POST_FAULT_HI].type = UINT;
    // Byte 4, 5: Run Fault Lo
    CVC_data[INVERTER_RUN_FAULT_LO].data = (frame.data[4]<<8) | frame.data[5];
    CVC_data[INVERTER_RUN_FAULT_LO].type = UINT;
    // Byte 6, 7: Run Fault Hi
    CVC_data[INVERTER_RUN_FAULT_HI].data = (frame.data[6]<<8) | frame.data[7];
    CVC_data[INVERTER_RUN_FAULT_HI].type = UINT;
    xSemaphoreGive(CVC_DataMutex);
}
/**
...
void CAN_Parse_Inverter_HighSpeedParameters(CAN_Queue_Frame_t frame){
    ...
    CVC_data[INVERTER_TORQUE_COMMAND_HI].data = (frame.data[0]<<8) | frame.data[1];
    CVC_data[INVERTER_TORQUE_COMMAND_HI].type = INT;
    // Byte 2, 3: Torque Feedback
    CVC_data[INVERTER_TORQUE_FEEDBACK_HI].data = (frame.data[2]<<8) | frame.data[3];
    CVC_data[INVERTER_TORQUE_FEEDBACK_HI].type = INT;
    // Byte 4, 5: Motor Speed
    CVC_data[INVERTER_MOTOR_SPEED_HI].data = (frame.data[4]<<8) | frame.data[5];
    CVC_data[INVERTER_MOTOR_SPEED_HI].type = INT;
    // Byte 6, 7: DC Bus Voltage
    CVC_data[INVERTER_DC_BUS_VOLTAGE_HI].data = (frame.data[6]<<8) | frame.data[7];
    CVC_data[INVERTER_DC_BUS_VOLTAGE_HI].type = INT;
    xSemaphoreGive(CVC_DataMutex);
}